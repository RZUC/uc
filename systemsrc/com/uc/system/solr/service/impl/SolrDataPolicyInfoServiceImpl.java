...
public class SolrDataPolicyInfoServiceImpl extends SolrDataCommonService<PolicyInfo>
{
    ...
    @Resource(name = "solrEventWeiXinDao")
    private SolrDao weixinSolrEvent;
    
    @Override
    public SolrDocumentList getSolrData(Query query, Page page)
    {
        SolrDocumentList list = new SolrDocumentList();
        SolrDocumentList weixinSolrYuQingList = weixinSolrYuQing.getData(query, page);
        SolrDocumentList weixinSolrEventList = weixinSolrEvent.getData(query, page);
        
        if (null != weixinSolrYuQingList)
        {
            list.addAll(weixinSolrYuQingList);
        }
        
        if (null != weixinSolrEventList)
        {
            list.addAll(weixinSolrEventList);
        }
    ...
    protected SolrDocumentList delDuplication(SolrDocumentList list)
    {
        ...
        for (SolrDocument doc : list)
        {
            ...
            // TODO:来源，标题，时间来去重
            String source = null == doc.get("source") ? "" : doc.get("source").toString();
            String title = null == doc.get("title") ? "" : doc.get("title").toString();
            String time = "";
    ...
    @Override
    protected PolicyInfo getHUserinfoByMap(SolrDocument doc, Map<String, PolicyInfo> map)
    {
        // TODO Auto-geRenH_Util.javanerated method stub
        return null;
    }
}
