...
public abstract class CommonDao implements SolrDao
{
    ...
    public static String TIMETYEP_Date = "date";
    ...
    public SolrDocumentList getData(Event event, Long rsid)
    {
        ...
        queryParams.setQuery(getParam(WordsCount.wordsCount(event.getWord())));
        
        queryParams.addFilterQuery(getFilterTime(event.getStartTime(), event.getEndTime()));
        
        if (rsid != null && rsid > 0 && null != getFilterRsid(rsid))
        {
            queryParams.addFilterQuery(getFilterRsid(rsid));
        }
    ...
    /**
     * @Title: getFilterByEvent
     * @Description: 根据事件中的属性来获取查询条件
     * @param @param event
     * @param @return 设定文件
     * @return String 返回类型
     */
    ...
    {
        log.debug("getFilterByEvent：{}", "空");
        return null;
    }
    
    /**
     * @Title: getFilterRsid
     * @Description: 添加Risd参数
     * @param @param rsid
     * @param @return 设定文件
     * @return String 返回类型
     */
    ...
    protected QueryResponse getQuery(HttpSolrServer solrServer, SolrQuery params)
    {
        for (int i = 1; i <= 3; i++)
        {
            try
            {
                ...
                log.debug("查询参数:[Url:{}]\t查询参数:{}", solrServer.getBaseURL(), params.getQuery());
                return response;
            }
            catch (SolrServerException e)
            {
                log.error("查询出错:[Url:{}]\t[重试次数：{}]\t[ERRORMessage:{}]\t[param:{}]",
    ...
    protected String getFilterTime(String startTime, String endTime)
    {
        ...
        if (TIMETYEP_Date.equals(getTimeType()))
        {
            time = SolrTool.processTimeDate(startTime, endTime);
            log.debug("Date 时间:{}", time);
        }
